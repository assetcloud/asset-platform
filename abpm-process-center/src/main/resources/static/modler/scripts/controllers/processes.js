...
angular.module('flowableModeler')
  .controller('ProcessesCtrl', ['$rootScope', '$scope', '$translate', '$http', '$timeout','$location', '$modal', 
	  ...
	  // 这个是在流程框上任意位置点击，然后显示正常的流程详情页面
	  $scope.showProcessDetails = function(process) {
          // alert(process.id);
	      if (process) {
	          $rootScope.editorHistory = [];
	          // alert("/processes/" + process.id);
              // 指向新地址，又被app.js中的路由捕获
	          $location.path("/processes/" + process.id);
	      }

	  };

	  //在processes.html页面中点击一个流程信息中的放大镜之后触发的效果，进入流程迁移页面
	  $scope.changeProcess = function(process) {
	      if (process) {
	          $rootScope.editorHistory = [];
	          $location.path("/changeprocesses/" + process.id);
	      }
	  };

...
public class ModelResource {
    ...
    protected ModelRepresentation updateModel(Model model, MultiValueMap<String, String> values, boolean forceNewVersion) {
        ...
        try {
            ...
            LOGGER.info("编辑的model是 {} ",model.getId());
            /* *
             * 回滚流程：
            ...
            String processIntanceId = "010b6c9c-4604-11e9-9f3d-00ffa337b651";
            String executionId = "010b6ca2-4604-11e9-9f3d-00ffa337b651";
            String taskToCancel = "teacher_permit";
            String taskToStart = "confirm_leave";

            //流程回滚
            LOGGER.info("开始处理流程 {} ",processIntanceId);
            runtimeService.createChangeActivityStateBuilder().processInstanceId(processIntanceId)
                    .moveActivityIdTo(taskToCancel,taskToStart).changeState();
            runtimeService.createChangeActivityStateBuilder().processInstanceId(processIntanceId)
                    .moveExecutionToActivityId(executionId,taskToStart).changeState();
            LOGGER.info("流程 {} 回滚完成",processIntanceId);
        }
    }

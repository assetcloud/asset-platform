...
import cn.org.assetcloud.system.utils.Func;
import cn.org.assetcloud.system.wrapper.SceneRoleWrapper;
...
public class SceneRoleController {
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(value = "current", defaultValue = "1", required = true, dataTypeClass = Integer.class),
    ...
    public R listAll(Query query, @ApiIgnore @RequestParam Map<String, Object> sceneRole){
        ...
        SceneRoleWrapper sceneRoleWrapper = new SceneRoleWrapper(sceneRoleService, roleGroupService, sceneService, dictClient);
    ...
    @PostMapping("submit")
    @ApiOperation(value = "业务角色新增或修改", notes = "（已完成）传入sceneRole实体")
    @ApiImplicitParams({
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "current", value = "页码", required = true, dataType = "String"),
...
//    }
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(name = "rids", value = "角色id的数组", required = true, dataType = "string")
    })

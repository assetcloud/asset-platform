...
@Component
public class JointCheckCommand {
    ...
    public void filtrate(List<AsRunningTask> runningTasks, LoginUser loginUser) {
        loop1:for(int i=0;i<runningTasks.size();i++){
            ...
            if(procNodeService.ifJointSign(task.getProcModelId(),task.getNodeId()))
            {
                ...
                List<AsFormInstDO> doList = formInstService.getExecutorId(task.getProcInstId(),loginUser.getUserId());
                for(int k = 0; k<doList.size(); k++)
                {
                    //当前用户已经执行过了当前会签任务
                    String tempNodeId = flowableService.getNodeId(doList.get(k).getTaskId());
...
//            throw new FormException("当前用户权限或者工作场景权限不够，无法获取当前任务节点信息！");
        ...
        if(is){
            List<String> taskId = formInstService.getAlreadyCompleteTask(formInstBO.getCurUserId(),
            ...
            for (int m = 0;m<taskId.size();m++){
                ...
                String nodeId = flowableService.getNodeId(taskId.get(m));
